...
namespace StatefullWorkflow.Engine
{
    public class WorkflowProcesser_Old
    {
        ...
        //public static StateMachine<State, string> ConfigureStateMachine(Workflow workflow, Func<State> stateAccessor, Action<State> stateMutator, IUnitOfWork unitOfWork)
        //{
        ...
        //    //var activityRepo = new StateActivityRepository(unitOfWork);
        //    var transitionRepo = new StateTransitionRepository(unitOfWork);
        //    var states = stateRepo.Where(s => s.WorkflowId == workflow.Id);
