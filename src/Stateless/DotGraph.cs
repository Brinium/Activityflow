            ...
            foreach (var stateCfg in _stateConfiguration) 
            {
                ...
                foreach (var behaviours in stateCfg.Value.TriggerBehaviours) 
                {
                    foreach (TriggerBehaviour behaviour in behaviours.Value) 
                    {
                        ...
                        string line = string.Format(" {0} -> {1} [label=\"{2} [{3}]\"];", source, destination, behaviour.Trigger, behaviour.GuardDescription); ;
                        //(behaviour.Guard.Method.DeclaringType.Namespace.Equals("Stateless")) ?
